...
namespace Renci.SshNet.Security.Cryptography.Ciphers
{
    ...
    public sealed class AesCipher : BlockCipher
    {
        ...
        {
            _c0 = LittleEndianToUInt32(bytes, off);
            _c1 = LittleEndianToUInt32(bytes, off + 4);
            _c2 = LittleEndianToUInt32(bytes, off + 8);
            _c3 = LittleEndianToUInt32(bytes, off + 12);
        }

        private void PackBlock(byte[] bytes, int off)
        {
            UInt32ToLittleEndian(_c0, bytes, off);
            UInt32ToLittleEndian(_c1, bytes, off + 4);
            UInt32ToLittleEndian(_c2, bytes, off + 8);
            UInt32ToLittleEndian(_c3, bytes, off + 12);
        }
